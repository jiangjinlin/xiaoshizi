    ...
    <nav class="fixed top-0 left-0 right-0 h-16 z-50 bg-white/80 backdrop-blur border-b border-gray-200">
      <div class="max-w-7xl mx-auto h-full px-6 flex items-center justify-between">
        <div class="flex items-center gap-6">
          ...
          <span class="hidden sm:inline text-lg font-semibold text-gray-700">专项练习 - 答题</span>
        </div>
        <div class="flex items-center gap-3 text-sm">
          <button class="px-4 h-10 rounded-lg text-gray-600 bg-white/60 border border-gray-200 hover:bg-white hover:text-gray-800 transition" @click="backPrev">返回学生页</button>
        </div>
      </div>
    </nav>
      ...
      <div class="rounded-2xl border border-gray-200 bg-white/70 backdrop-blur shadow-sm overflow-hidden">
        <div class="p-6 border-b border-gray-200 flex flex-col sm:flex-row gap-4 sm:items-center sm:justify-between">
          ...
          <div class="flex flex-wrap gap-3">
            ...
            <button class="h-10 px-4 rounded-lg bg-white/70 text-gray-700 text-xs font-medium border border-gray-300 hover:bg-white transition" @click="backToSetup">重新出题</button>
          </div>
        </div>
            ...
            <div class="flex items-start gap-4">
              ...
              <div class="flex-1 min-w-0">
                <div class="mb-3">
                  ...
                  <a v-if="currentQuestion.type==='操作' && currentQuestion.template_url" :href="currentQuestion.template_url" target="_blank" class="ml-2 text-primary text-xs hover:underline">下载模板</a>
                </div>
                ...
                <!-- 操作 -->
                <div v-else class="text-xs text-gray-500">操作题不参与自动判题，请下载模板并自行练习。</div>
        ...
        <div v-if="currentQuestion" class="p-6 border-t border-gray-200 bg-white/60">
          <div class="flex items-center justify-between">
            ...
            <div class="text-[11px] text-gray-500" v-if="!canShowComments">提交答案后可查看与发表本题评论</div>
          </div>
                ...
                <li v-for="c in qComments" :key="c.id" class="p-3 rounded-lg border border-gray-200 bg-white/70">
                  <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-2">
                      ...
                      <button v-if="c.can_delete" class="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded border border-red-200 text-red-600 bg-red-50 hover:bg-red-100"
                              @click="deleteQComment(c)">
...
const resultById = computed(() => { const m = {}; for (const d of resultDetails.value) m[d.id] = d; return m })
// 当前题是否已判题（作为评论区显示条件）
const canShowComments = computed(() => {
  const q = currentQuestion.value
  if (!q) return false
  return !!resultById.value[q.id]
})
...
function restoreQuestions() {
  try {
    const qStr = sessionStorage.getItem('practice_questions')
    const fStr = sessionStorage.getItem('practice_filters')
    if (!qStr) {
      router.replace('/practice/setup')
      return
    }
    const list = JSON.parse(qStr)
    questions.value = Array.isArray(list) ? list : []
    rawFilters.value = fStr ? JSON.parse(fStr) : null
...
const progressPercent = computed(() => {
  ...
  return Math.min(100, Math.round(100 * answeredCount.value / total))
})
...
function isCorrect(idx){
  const q = questions.value[idx]
  const d = q ? resultById.value[q.id] : null
  return !!(d && d.is_correct)
}
...
function cardClass(idx){
  if (idx === currentIndex.value) return 'bg-primary text-white border-primary'
  if (isCorrect(idx)) return 'bg-green-100 text-green-700 border-green-200 hover:bg-green-200'
  if (isWrong(idx)) return 'bg-red-100 text-red-700 border-red-200 hover:bg-red-200'
  if (isAnswered(idx)) return 'bg-amber-50 text-amber-700 border-amber-200 hover:bg-amber-100'
...
async function loadQComments(){
  ...
  try {
    ...
    console.warn('加载题目评论失败', e._friendly || e?.message)
  } finally {
    qCommentsLoading.value = false
  }
}
async function postQComment(){
  ...
  try {
    ...
    alert(e._friendly || '发布失败，请稍后再试')
  } finally {
    qCommentPosting.value = false
  }
}
...
async function toggleLike(c){
  ...
  try {
    ...
    if (data?.success){
      ...
      // 重新按点赞数排序
      qComments.value = [...qComments.value].sort((a,b)=>{
        const la = (a.likes||0), lb = (b.likes||0)
        if (lb !== la) return lb - la
        return (b.id||0) - (a.id||0)
      })
    ...
    alert(e._friendly || '操作失败，请稍后再试')
  } finally {
    likePending[c.id] = false
  }
}
...
async function deleteQComment(c){
  ...
  try{
    const { data } = await apiQCommentsDelete(c.id)
    if (data?.success){
      qComments.value = qComments.value.filter(x => x.id !== c.id)
    }
  }catch(e){
    alert(e._friendly || '删除失败，请稍后再试')
  }
}

watch([currentQuestion, canShowComments], () => { loadQComments() })

onMounted(() => { restoreQuestions() })
</script>
